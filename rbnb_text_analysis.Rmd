...
plot_pca<- function(df,feat){
 print(autoplot(pca.pca, data = df.pca,colour=feat,alpha=.4))
...
```{r}
levels_df<- df %>%  
     sapply(nlevels)%>% as.data.frame() 
levels_df <-levels_df%>% mutate(fearure = rownames(levels_df))
colnames(levels_df)<- c("num_features")
levels_df %>% arrange(desc(num_features))
