...
ggplot(data=df_neighbourhood ,aes(x=date,y=total_offers,colour=neighbourhood_group))+ geom_line()+ theme_economist()+scale_color_economist(stata = TRUE)
ggplot(data=df_neighbourhood,aes(x=date,y=mean_price,colour=neighbourhood_group))+ geom_line()+scale_colour_brewer(palette = "Set1")+ theme_economist()
```
```{r}
df.may2017<-read.csv('data/listings/2017_05.csv')

df.mitte.2017 <- df.may2017 %>% filter(neighbourhood_group=='Mitte')
groupy.mitte.may.2017<- mitte %>% ungroup() %>% group_by(neighbourhood) %>% summarise(total= n())
...
ggplot(data=df_neighbourhood.mitte ,aes(x=date,y=total_offers,colour=neighbourhood))+ geom_line()+ theme_economist()+scale_color_economist(stata = TRUE)
ggplot(data=df_neighbourhood.mitte,aes(x=date,y=mean_price,colour=neighbourhood))+ geom_line()+theme_economist()+scale_color_economist(stata = TRUE)
```
