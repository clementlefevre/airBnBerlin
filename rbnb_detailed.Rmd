...
df <- df %>% filter(room_type=='Entire home/apt') %>% filter(grepl('Mitte',neighbourhood_group_cleansed))
```
...
df$quartile <- with(df, cut(prico/beds, 
   ...
   ggmap(ggmap_berlin)+ geom_point(aes(x=longitude, y=latitude,colour=quartile), data=df)+ scale_colour_brewer(palette="BuGn")
```
```{r}
